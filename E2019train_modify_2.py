...
# 加载训练好的模型权重
model_dict_path = '/root/autodl-tmp/code/EyeCRNet/train_model_gelu/model_epoch_98_loss_0.0572.pth'

# 初始化模型
model = model.Unet(1, 4).to(device)  # 将模型移动到正确的设备
model.load_state_dict(torch.load(model_dict_path, map_location=device))  # 加载权重到相同的设备
model.eval()
...
for epoch in range(num_epoch):
    ...
    if avg_epoch_loss < lowest_loss:
        ...
        # 保存当前回合的模型参数
        last_saved_model_path = f"/root/autodl-tmp/code/EyeCRNet/gelu1tun/model_epoch_{epoch+1}_loss_{avg_epoch_loss:.4f}.pth"
